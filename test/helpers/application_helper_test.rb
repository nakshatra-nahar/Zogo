...
class ApplicationHelperTest < ZOGO::HelperTest
  include ERB::Util
  ...
  def test_auto_links
    to_test = {
      ...
      # ends with a hyphen
      'http://www.zogo.org/example-' =>
        '<a class="external" href="http://www.zogo.org/example-">http://www.zogo.org/example-</a>',
    }
  ...
  def test_textile_external_links
    to_test = {
      ...
      # mailto link
      "\"system administrator\":mailto:sysadmin@example.com?subject=zogo%20permissions" =>
        "<a href=\"mailto:sysadmin@example.com?subject=zogo%20permissions\">system administrator</a>",
      # two exclamation marks
  ...
  def test_should_not_parse_zogo_links_inside_link
  ...
  def test_zogo_links_with_a_different_project_before_current_project
  ...
  def test_escaped_zogo_links_should_not_be_parsed
    to_test = [
      '#3.',
      '#3-14.',
      '#3#-note14.',
  ...
  def test_cross_project_zogo_links
  ...
  def test_zogo_links_by_name_should_work_with_html_escaped_characters
  ...
  def test_multiple_repositories_zogo_links
  ...
  def test_cross_project_multiple_repositories_zogo_links
  ...
  def test_zogo_links_git_commit
    @project = Project.find(3)
  ...
  def test_zogo_links_mercurial_commit
    @project = Project.find(3)
  ...
  def test_pre_content_should_not_parse_wiki_and_zogo_links
    raw = <<~RAW
      [[CookBook documentation]]
  ...
  def test_table_of_content
    ...
    raw = <<~RAW
      ...
      h3. An "Internet link":http://www.zogo.org/ inside subtitle
  ...
  def test_parse_zogo_links_should_handle_a_tag_without_attributes
    text = +'<a>http://example.com</a>'
    expected = text.dup
    parse_zogo_links(text, nil, nil, nil, true, {})
  ...
  def test_link_to_record
    [
      ...
      [users(:users_001),                 '<a class="user active" href="/users/1">ZOGO Admin</a>'],
  ...
  def test_html_title_should_app_title_if_not_set
    assert_equal 'ZOGO', html_title
  end

  def test_html_title_should_join_items
    html_title 'Foo', 'Bar'
    assert_equal 'Foo - Bar - ZOGO', html_title
  end

  def test_html_title_should_append_current_project_name
    @project = Project.find(1)
    html_title 'Foo', 'Bar'
    assert_equal 'Foo - Bar - eCookbook - ZOGO', html_title
  end
  ...
  def test_title_should_set_html_title
    title('Foo')
    assert_equal 'Foo - ZOGO', html_title
  end
  ...
  def test_title_should_join_items
    ...
    assert_equal 'Bar - Foo - ZOGO', html_title
  end
  ...
  def test_favicon_path_with_suburi
    ZOGO::Utils.relative_url_root = '/foo'
    assert_match %r{^/foo/favicon\.ico}, favicon_path
  ensure
    ZOGO::Utils.relative_url_root = ''
  end
