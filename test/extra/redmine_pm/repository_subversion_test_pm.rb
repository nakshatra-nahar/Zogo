...
class ZOGOPmTest::RepositorySubversionTest < ZOGOPmTest::TestCase
  fixtures :projects, :users, :members, :roles, :member_roles, :auth_sources, :enabled_modules

  SVN_BIN = ZOGO::Configuration['scm_subversion_command'] || "svn"
  ...
  if ldap_configured?
    def test_user_with_ldap_auth_source_should_authenticate_with_ldap_credentials
      ldap_user = User.new(:mail => 'example1@zogo.org', :firstname => 'LDAP', :lastname => 'user', :auth_source_id => 1)
  ...
  def svn_url(path=nil)
    host = ENV['ZOGO_TEST_DAV_SERVER'] || '127.0.0.1'
