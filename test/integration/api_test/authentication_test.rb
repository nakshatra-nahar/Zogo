...
class ZOGO::ApiTest::AuthenticationTest < ZOGO::ApiTest::Base
  fixtures :users
  ...
  def test_api_should_accept_auth_using_api_key_as_request_header
    user = User.generate!
    token = Token.create!(:user => user, :action => 'api')
    get "/users/current.xml", :headers => {'X-ZOGO-API-Key' => token.value.to_s}
    assert_response 200
  end
  ...
  def test_api_should_accept_switch_user_header_for_admin_user
    ...
    get '/users/current', :headers => {'X-ZOGO-API-Key' => user.api_key, 'X-ZOGO-Switch-User' => su.login}
    assert_response :success
    assert_select 'h2', :text => su.name
  end

  def test_api_should_respond_with_412_when_trying_to_switch_to_a_invalid_user
    get '/users/current', :headers => {'X-ZOGO-API-Key' => User.find(1).api_key, 'X-ZOGO-Switch-User' => 'foobar'}
    assert_response 412
  end
  ...
  def test_api_should_respond_with_412_when_trying_to_switch_to_a_locked_user
    ...
    get '/users/current', :headers => {'X-ZOGO-API-Key' => User.find(1).api_key, 'X-ZOGO-Switch-User' => user.login}
    assert_response 412
  end
