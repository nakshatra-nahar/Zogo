...
class AccountTest < ZOGO::IntegrationTest
  ...
  def test_autologin_should_use_autologin_cookie_name
    Token.delete_all
    ZOGO::Configuration.stubs(:[]).with('autologin_cookie_name').returns('custom_autologin')
    ZOGO::Configuration.stubs(:[]).with('autologin_cookie_path').returns('/')
    ZOGO::Configuration.stubs(:[]).with('autologin_cookie_secure').returns(false)
    ZOGO::Configuration.stubs(:[]).with('sudo_mode_timeout').returns(15)
