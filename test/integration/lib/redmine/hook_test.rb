...
class HookTest < ZOGO::IntegrationTest
  ...
  class SidebarContent < ZOGO::Hook::ViewListener
    def view_layouts_base_sidebar(context)
      content_tag('p', 'Sidebar hook')
    end
  end

  ZOGO::Hook.clear_listeners

  class ContentForInsideHook < ZOGO::Hook::ViewListener
  ...
  def test_html_head_hook_response
    ZOGO::Hook.add_listener(ProjectBasedTemplate)
  ...
  def test_sidebar_with_hook_content_should_not_be_hidden
    ZOGO::Hook.add_listener(SidebarContent)
  ...
  def test_hook_with_content_for_should_append_content
    ZOGO::Hook.add_listener(ContentForInsideHook)
  ...
  def test_controller_hook_context_should_include_request
    ZOGO::Hook.add_listener(ContextTestHook)
  ...
  def test_multiple_hooks
    ZOGO::Hook.add_listener(SingleRenderOn)
    ZOGO::Hook.add_listener(MultipleRenderOn)
    get '/'
