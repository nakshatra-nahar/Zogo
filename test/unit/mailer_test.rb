...
class MailerTest < ActiveSupport::TestCase
  ...
  def test_generated_links_with_prefix
    relative_url_root = ZOGO::Utils.relative_url_root
  ...
  def test_generated_links_with_port_and_prefix
    with_settings :host_name => '10.0.0.1:81/zogo', :protocol => 'http' do
      Mailer.test_email(User.find(1)).deliver_now
      mail = last_email
      assert_include 'http://10.0.0.1:81/zogo', mail_body(mail)
    end
  end
  ...
  def test_generated_links_with_prefix_and_no_relative_url_root
    relative_url_root = ZOGO::Utils.relative_url_root
    ZOGO::Utils.relative_url_root = nil
    ...
    # restore it
    ZOGO::Utils.relative_url_root = relative_url_root
  end
  ...
  def test_email_headers
    with_settings :mail_from => 'ZOGO <zogo@example.net>' do
      issue = Issue.find(1)
    ...
    # List-Id should not include the display name "ZOGO"
    assert_equal '<zogo.example.net>', mail.header['List-Id'].to_s
    assert_equal 'Bug', mail.header['X-ZOGO-Issue-Tracker'].to_s
  end
  ...
  def test_email_headers_should_include_sender
    ...
    Mailer.deliver_issue_add(issue)
    mail = last_email
    assert_equal issue.author.login, mail.header['X-ZOGO-Sender'].to_s
  end
  ...
  def test_email_headers_should_not_include_assignee_when_not_assigned
    ...
    mail = last_email
    assert_not mail.header['X-ZOGO-Issue-Assignee']
  end
  ...
  def test_email_headers_should_include_assignee_when_assigned
    ...
    mail = last_email
    assert_equal 'jsmith', mail.header['X-ZOGO-Issue-Assignee'].to_s
  end
  ...
  def test_email_headers_should_include_assignee_if_assigned_to_group
    ...
    mail = last_email
    assert_equal 'Group (A Team)', mail.header['X-ZOGO-Issue-Assignee'].to_s
  end
  ...
  def test_from_header
    with_settings :mail_from => 'zogo@example.net' do
      Mailer.deliver_test_email(User.find(1))
    end
    mail = last_email
    assert_equal 'zogo@example.net', mail.from_addrs.first
  end

  def test_from_header_with_phrase
    with_settings :mail_from => 'ZOGO app <zogo@example.net>' do
      Mailer.deliver_test_email(User.find(1))
    end
    mail = last_email
    assert_equal 'zogo@example.net', mail.from_addrs.first
    assert_equal 'ZOGO app <zogo@example.net>', mail.header['From'].to_s
  end
  ...
  def test_from_header_with_rfc_non_compliant_phrase
    ...
    mail = last_email
    assert_match /<zogo@example\.net>/, mail.from_addrs.first
    assert_equal '[ZOGO app] <zogo@example.net>', mail.header['From'].to_s
  end
  ...
  def test_from_header_with_author_name
    ...
    with_settings :mail_from => 'zogo@example.net', :app_title => 'Foo' do
      ...
      mail = last_email
      assert_equal 'zogo@example.net', mail.from_addrs.first
      assert_equal 'Dave Lopper <zogo@example.net>', mail.header['From'].to_s

      # Use app_title if @author is nil or AnonymousUser
      Mailer.deliver_test_email(User.find(1))
      mail = last_email
      assert_equal 'zogo@example.net', mail.from_addrs.first
      assert_equal "Foo <zogo@example.net>", mail.header['From'].to_s
    end
  end
  ...
  def test_issue_add_message_id
    ...
    assert_include "zogo.issue-2.20060719190421.#{uid}@example.net", mail.message_id
    assert_include "zogo.issue-2.20060719190421.#{uid}@example.net", mail.references
  end
  ...
  def test_issue_edit_message_id
    ...
    mail = last_email
    uid = destination_user(mail).id
    assert_match /^zogo\.journal-3\.\d+\.#{uid}@example\.net/, mail.message_id
    assert_include "zogo.issue-2.20060719190421.#{uid}@example.net", mail.references
    assert_select_email do
      # link to the update
      assert_select "a[href=?]",
  ...
  def test_message_posted_message_id
    ...
    mail = last_email
    uid = destination_user(mail).id
    assert_include "zogo.message-1.20070512151532.#{uid}@example.net", mail.message_id
    assert_include "zogo.message-1.20070512151532.#{uid}@example.net", mail.references
    assert_select_email do
      # link to the message
      assert_select "a[href=?]",
  ...
  def test_reply_posted_message_id
    ...
    mail = last_email
    uid = destination_user(mail).id
    assert_include "zogo.message-3.20070512151802.#{uid}@example.net", mail.message_id
    assert_include "zogo.message-1.20070512151532.#{uid}@example.net", mail.references
    assert_select_email do
      # link to the reply
      assert_select "a[href=?]",
  ...
  def test_timestamp_in_message_id_should_be_utc
    ...
    %w(UTC Paris Tokyo).each do |zone|
      Time.use_zone(zone) do
        assert_match /^zogo\.issue-3\.20060719190727\.1@example\.net/, Mailer.token_for(issue, user)
      end
    end
  end
  ...
  def test_token_for_should_strip_trailing_gt_from_address_with_full_name
    with_settings :mail_from => "ZOGO Mailer<no-reply@zogo.org>" do
      assert_match /\Azogo.issue-\d+\.\d+\.3@zogo.org\z/,
