...
class ActivityTest < ActiveSupport::TestCase
  ...
  def test_user_activity
    user = User.find(2)
    events = ZOGO::Activity::Fetcher.new(User.anonymous, :author => user).events(nil, nil, :limit => 10)
  ...
  def test_journal_with_notes_and_changes_should_be_returned_once
    f = ZOGO::Activity::Fetcher.new(User.anonymous, :project => Project.find(1))
    f.scope = ['issues']
    events = f.events
  ...
  def test_event_types_should_consider_activity_provider_permission
    ZOGO::Activity.register 'test', :class_name => 'ActivityTest::TestActivityProviderWithPermission'
    user = MockUser.new(:custom_permission)
    f = ZOGO::Activity::Fetcher.new(user, :project => Project.find(1))
    assert_include 'test', f.event_types
  ensure
    ZOGO::Activity.delete 'test'
  end

  def test_event_types_should_include_activity_provider_with_nil_permission
    ZOGO::Activity.register 'test', :class_name => 'ActivityTest::TestActivityProviderWithNilPermission'
    user = MockUser.new
    f = ZOGO::Activity::Fetcher.new(user, :project => Project.find(1))
    assert_include 'test', f.event_types
  ensure
  ...
  def test_event_types_should_use_default_permission_for_activity_provider_without_permission
    ZOGO::Activity.register 'test', :class_name => 'ActivityTest::TestActivityProviderWithoutPermission'
  ...
  def find_events(user, options={})
    ZOGO::Activity::Fetcher.new(user, options).events(Date.today - 30, Date.today + 1)
  end
end
