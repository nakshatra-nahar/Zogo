...
class ZOGO::UnifiedDiffTest < ActiveSupport::TestCase
  def test_subversion_diff
    diff = ZOGO::UnifiedDiff.new(read_diff_fixture('subversion.diff'))
    # number of files
  ...
  def test_truncate_diff
    diff = ZOGO::UnifiedDiff.new(read_diff_fixture('subversion.diff'), :max_lines => 20)
    assert_equal 2, diff.size
  end

  def test_inline_partials
    diff = ZOGO::UnifiedDiff.new(read_diff_fixture('partials.diff'))
  ...
  def test_side_by_side_partials
    diff = ZOGO::UnifiedDiff.new(read_diff_fixture('partials.diff'), :type => 'sbs')
    assert_equal 1, diff.size
    diff = diff.first
  ...
  def test_partials_with_html_entities
    ...
    DIFF
    diff = ZOGO::UnifiedDiff.new(raw, :type => 'sbs')
    assert_equal 1, diff.size
    ...
    diff = ZOGO::UnifiedDiff.new(raw, :type => 'inline')
    assert_equal 1, diff.size
  ...
  def test_line_starting_with_dashes
    diff = ZOGO::UnifiedDiff.new(<<~DIFF)
  ...
  def test_utf8_ja
    with_settings :repositories_encodings => '' do
      diff = ZOGO::UnifiedDiff.new(read_diff_fixture('issue-12641-ja.diff'), :type => 'inline')
      assert_equal 1, diff.size
  ...
  def test_utf8_ru
    with_settings :repositories_encodings => '' do
      diff = ZOGO::UnifiedDiff.new(read_diff_fixture('issue-12641-ru.diff'), :type => 'inline')
      assert_equal 1, diff.size
  ...
  def test_git_footer_line
    ...
    diff = ZOGO::UnifiedDiff.new(body_lines.join("\n") + "\n", :type => 'sbs')
    ...
    diff = ZOGO::UnifiedDiff.new("test\n", :type => 'sbs')
