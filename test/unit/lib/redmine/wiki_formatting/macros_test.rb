...
class ZOGO::WikiFormatting::MacrosTest < ZOGO::HelperTest
  ...
  def test_macro_registration
    ZOGO::WikiFormatting::Macros.register do
  ...
  def test_macro_name_with_upper_case
    with_settings :text_formatting => 'textile' do
      ZOGO::WikiFormatting::Macros.macro(:UpperCase) {|obj, args| 'Upper'}
      assert_equal '<p>Upper</p>', textilizable('{{UpperCase}}')
    end
  end

  def test_multiple_macros_on_the_same_line
    ZOGO::WikiFormatting::Macros.macro :foo do |obj, args|
  ...
  def test_macro_exception_should_be_displayed
    ZOGO::WikiFormatting::Macros.macro :exception do |obj, args|
      raise "My message"
    end
  ...
  def test_macro_arguments_should_not_be_parsed_by_formatters
    text = '{{hello_world(http://www.zogo.org, #1)}}'
    assert_include 'Arguments: http://www.zogo.org, #1', textilizable(text)
  end
  ...
  def test_html_safe_macro_output_should_not_be_escaped
    with_settings :text_formatting => 'textile' do
      ZOGO::WikiFormatting::Macros.macro :safe_macro do |obj, args|
        '<tag>'.html_safe
      end
