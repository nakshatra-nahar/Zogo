...
class ZOGO::SyntaxHighlighting::RougeTest < ActiveSupport::TestCase
  def test_filename_supported
    ...
    to_test.each do |filename, expected|
      assert_equal expected, ZOGO::SyntaxHighlighting::Rouge.filename_supported?(filename)
    end
  end
  ...
  def test_highlight_by_filename_should_distinguish_perl_and_prolog
    ...
    assert_equal expected_perl, ZOGO::SyntaxHighlighting::Rouge.highlight_by_filename(raw_perl, filename)
    assert_equal expected_prolog, ZOGO::SyntaxHighlighting::Rouge.highlight_by_filename(raw_prolog, filename)
  end
end
