...
class ZOGO::MenuManager::MenuHelperTest < ZOGO::HelperTest
  include ZOGO::MenuManager::MenuHelper
  include ERB::Util
  ...
  def test_render_single_menu_node
    node = ZOGO::MenuManager::MenuItem.new(:testing, '/test', {})
  ...
  def test_render_menu_node
    single_node = ZOGO::MenuManager::MenuItem.new(:single_node, '/test', {})
  ...
  def test_render_menu_node_with_symbol_as_url
    node = ZOGO::MenuManager::MenuItem.new(:testing, :issues_path)
  ...
  def test_render_menu_node_with_symbol_as_url_and_project
    node = ZOGO::MenuManager::MenuItem.new(:testing, :project_issues_path)
  ...
  def test_render_menu_node_with_nested_items
    parent_node = ZOGO::MenuManager::MenuItem.new(:parent_node, '/test', {})
    parent_node << ZOGO::MenuManager::MenuItem.new(:child_one_node, '/test', {})
    parent_node << ZOGO::MenuManager::MenuItem.new(:child_two_node, '/test', {})
    parent_node <<
      ZOGO::MenuManager::MenuItem.new(:child_three_node, '/test', {}) <<
      ZOGO::MenuManager::MenuItem.new(:child_three_inner_node, '/test', {})
        ...
        new(
          ...
          '/test',
          {
            :children => Proc.new do |p|
              children = []
              3.times do |time|
                children << ZOGO::MenuManager::MenuItem.new(
                  "test_child_#{time}",
  ...
  def test_render_menu_node_with_allowed_and_unallowed_standard_children
    ...
    ZOGO::MenuManager.map :some_menu do |menu|
  ...
  def test_render_empty_virtual_menu_node_with_children
    ...
    ZOGO::MenuManager.map :menu1 do |menu|
      menu.push(:parent_node, nil, {})
    end

    # parent with unallowed unattached child
    ZOGO::MenuManager.map :menu2 do |menu|
      menu.push(
        :parent_node, nil,
        {
          :children =>
    ...
    ZOGO::MenuManager.map :menu3 do |menu|
      menu.push(:parent_node, nil, {})
      menu.push(:test_child_unallowed, {:controller =>'issues', :action => 'new'}, {:parent => :parent_node})
    end
    ...
    parent_node =
      ZOGO::MenuManager::MenuItem.new(
        ...
        '/test',
        {
          :children =>
            Proc.new do |p|
              ZOGO::MenuManager::MenuItem.new("test_child", "/testing", {})
            end
        }
      )
    assert_raises ZOGO::MenuManager::MenuError, ":children must be an array of MenuItems" do
      @output_buffer = render_menu_node(parent_node, Project.find(1))
    end
  end
  ...
  def test_menu_items_for_should_yield_all_items_if_passed_a_block
    ...
    ZOGO::MenuManager.map menu_name do |menu|
      menu.push(:a_menu, '/', {})
      menu.push(:a_menu_2, '/', {})
      menu.push(:a_menu_3, '/', {})
    end
