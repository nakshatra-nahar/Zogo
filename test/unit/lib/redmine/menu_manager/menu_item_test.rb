...
class ZOGO::MenuManager::MenuItemTest < ActiveSupport::TestCase
  include ZOGOMenuTestHelper

  ZOGO::MenuManager.map :test_menu do |menu|
  ...
  def test_new_menu_item_should_require_an_url
    assert_raises ArgumentError do
      ZOGO::MenuManager::MenuItem.new(:test_missing_url)
    end
  end

  def test_new_menu_item_with_all_required_parameters
    assert ZOGO::MenuManager::MenuItem.new(:test_good_menu, '/test', {})
  end
