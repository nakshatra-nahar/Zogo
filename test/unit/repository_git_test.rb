...
class RepositoryGitTest < ActiveSupport::TestCase
  ...
  REPOSITORY_PATH.tr!('/', "\\") if ZOGO::Platform.mswin?

  REPOSITORY_UTF8_PATH = Rails.root.join('tmp/test/git_utf8_repository').to_s
  REPOSITORY_UTF8_PATH.tr!('/', "\\") if ZOGO::Platform.mswin?
  ...
  if File.directory?(REPOSITORY_PATH)
    ...
    def test_entries
      ...
      assert_kind_of ZOGO::Scm::Adapters::Entries, entries
    end
  ...
  if File.directory?(REPOSITORY_UTF8_PATH) &&
      !(ZOGO::Database.mysql? && !is_mysql_utf8mb4)
    def test_utf8_emoji
      repo =
        Repository::Git.create(
          :project      => @project,
