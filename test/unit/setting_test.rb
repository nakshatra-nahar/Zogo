...
class SettingTest < ActiveSupport::TestCase
  ...
  def test_read_default
    ...
    assert_equal "ZOGO", Setting.app_title
  ...
  def test_mail_from_format_should_be_validated
    with_locale 'en' do
      ['[ZOGO app] <zogo@example.net>', 'zogo'].each do |invalid_mail_from|
        errors = Setting.set_all_from_params({:mail_from => invalid_mail_from})
        assert_includes errors, [:mail_from, 'is invalid']
      end

      ['ZOGO app <zogo@example.net>', 'zogo@example.net', '<zogo@example.net>'].each do |valid_mail_from|
