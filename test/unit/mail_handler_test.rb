...
class MailHandlerTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    Setting.notified_events = ZOGO::Notifiable.all.collect(&:name)
    User.current = nil
  end
  ...
  def test_add_issue_to_project_specified_by_subaddress
    # This email has zogo+onlinestore@somenet.foo as 'To' header
    issue =
      submit_email(
        'ticket_on_project_given_by_to_header.eml',
        :issue => {:tracker => 'Support request'},
        :project_from_subaddress => 'zogo@somenet.foo'
      )
    assert issue.is_a?(Issue)
    assert !issue.new_record?
    issue.reload
  ...
  def test_should_ignore_emails_from_emission_address
    emission_addresses = [
      'zogo@example.net',
      'ZOGO <zogo@example.net>',
      'zogo@example.net (ZOGO)'
    ]
    Role.anonymous.add_permission!(:add_issues)
  ...
  def test_replying_to_a_private_note_should_add_reply_as_private
    ...
    assert_difference 'Journal.count' do
      journal = submit_email('ticket_reply.eml') do |email|
        email.sub! %r{^In-Reply-To:.*$}, "In-Reply-To: <zogo.journal-#{private_journal.id}.20060719210421@osiris>"
      end
  ...
  def test_reply_to_a_nonexitent_journal
    ...
    assert_no_difference 'Issue.count' do
      assert_no_difference 'Journal.count' do
        journal = submit_email('ticket_reply.eml') do |email|
          email.sub! %r{^In-Reply-To:.*$}, "In-Reply-To: <zogo.journal-#{journal_id}.20060719210421@osiris>"
        end
        assert_nil journal
      end
    end
  ...
  def test_should_handle_outlook_web_access_2010_html_only
    ...
    assert issue.is_a?(Issue)
    issue.reload
    assert_equal 'Upgrade ZOGO to 3.0.x', issue.subject
  ...
  def test_should_handle_outlook_2010_html_only
    ...
    assert_equal(
      "Simple, unadorned test email generated by Outlook 2010. It is in HTML format, but" \
        ...
        " drop it into the Inbox for scraping by ZOGO 3.0.2.",
      issue.description
    )
  end
