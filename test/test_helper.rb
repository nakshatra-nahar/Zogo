...
if ENV["COVERAGE"]
  ...
  SimpleCov.formatter = ZOGO::Coverage::HtmlFormatter
  SimpleCov.start 'rails'
end

$zogo_test_ldap_server = ENV['ZOGO_TEST_LDAP_SERVER'] || '127.0.0.1'
...
ZOGO::SudoMode.disable!

$zogo_tmp_attachments_directory = "#{Rails.root}/tmp/test/attachments"
FileUtils.mkdir_p $zogo_tmp_attachments_directory

$zogo_tmp_pdf_directory = "#{Rails.root}/tmp/test/pdf"
FileUtils.mkdir_p $zogo_tmp_pdf_directory
FileUtils.rm Dir.glob('#$zogo_tmp_pdf_directory/*.pdf')
...
class ActiveSupport::TestCase
  ...
  def set_tmp_attachments_directory
    Attachment.storage_path = $zogo_tmp_attachments_directory
  end
  ...
  def self.ldap_configured?
    @test_ldap = Net::LDAP.new(:host => $zogo_test_ldap_server, :port => 389)
  ...
  def self.convert_installed?
    ZOGO::Thumbnail.convert_available?
  end
  ...
  def self.is_mysql_utf8mb4
    return false unless ZOGO::Database.mysql?
...
module ZOGO
  ...
  class ControllerTest < ActionController::TestCase
    ...
    def save_pdf
      ...
      File.binwrite(File.join($zogo_tmp_pdf_directory, filename), response.body)
    end
  end
  ...
  module ApiTest
    ...
    class Base < ZOGO::IntegrationTest
      def setup
    ...
    class Routing < ZOGO::RoutingTest
      def should_route(arg)
        arg = arg.dup
        request = arg.keys.detect {|key| key.is_a?(String)}
        raise ArgumentError unless request
