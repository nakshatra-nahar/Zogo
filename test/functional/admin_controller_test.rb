...
class AdminControllerTest < ZOGO::ControllerTest
  ...
  def test_load_default_configuration_data_should_rescue_error
    delete_configuration_data
    ZOGO::DefaultData::Loader.stubs(:load).raises(StandardError.new("Something went wrong"))
    post(
      :default_configuration,
      :params => {
        :lang => 'fr'
  ...
  def test_no_plugins
    ZOGO::Plugin.stubs(:registered_plugins).returns({})
  ...
  def test_admin_menu_plugin_extension
    ZOGO::MenuManager.map :admin_menu do |menu|
