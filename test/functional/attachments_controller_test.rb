...
class AttachmentsControllerTest < ZOGO::ControllerTest
  ...
  def test_download_should_assign_application_octet_stream_if_content_type_is_not_determined
    ...
    assert_response :success
    assert_nil ZOGO::MimeType.of(attachments(:attachments_022).filename)
  ...
  if convert_installed?
    ...
    def test_thumbnail_should_not_exceed_maximum_size
      ZOGO::Thumbnail.expects(:generate).with {|source, target, size| size == 800}
      @request.session[:user_id] = 2
      get(
        :thumbnail,
        :params => {
    ...
    def test_thumbnail_should_round_size
      ZOGO::Thumbnail.expects(:generate).with {|source, target, size| size == 300}
      @request.session[:user_id] = 2
      get(
        :thumbnail,
        :params => {
