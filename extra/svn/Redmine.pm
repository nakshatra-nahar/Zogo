package Apache::Authn::ZOGO;

=head1 Apache::Authn::ZOGO

ZOGO - a mod_perl module to authenticate webdav subversion users
against zogo database

=head1 SYNOPSIS

This module allow anonymous users to browse public project and
registred users to browse and commit their project. Authentication is
done against the zogo database or the LDAP configured in zogo.
...
If your ZOGO users use LDAP authentication, you will also need
...
=head1 CONFIGURATION
   ...
   ## eg:  /usr/lib/perl5/Apache/Authn/ZOGO.pm
   PerlLoadModule Apache::Authn::ZOGO
   <Location /svn>
     DAV svn
     ...
     PerlAccessHandler Apache::Authn::ZOGO::access_handler
     PerlAuthenHandler Apache::Authn::ZOGO::authen_handler

     ## for mysql
     ZOGODSN "DBI:mysql:database=databasename;host=my.db.server"
     ## for postgres
     # ZOGODSN "DBI:Pg:dbname=databasename;host=my.db.server"

     ZOGODbUser "zogo"
     ZOGODbPass "password"
     ## Optional where clause (fulltext search would be slow and
     ## database dependent).
     # ZOGODbWhereClause "and members.role_id IN (1,2)"
     ## Optional credentials cache size
     # ZOGOCacheCredsMax 50
  </Location>

To be able to browse repository inside zogo, you must add something
like that :
   ...
   <Location /svn-private>
     ...
     <Limit GET PROPFIND OPTIONS REPORT>
       Allow from zogo.server.ip
     </Limit>
   </Location>

and you will have to use this reposman.rb command line to create repository :

  reposman.rb --zogo my.zogo.server --svn-dir /var/svn --owner www-data -u http://svn.server/svn-private/
...
above settings. ZOGO.pm normally does access control depending on the HTTP
...
a MySQL database and this new option:
   ...
   <Location /git>
       ...
       PerlAuthenHandler Apache::Authn::ZOGO::authen_handler
       # for mysql
       ZOGODSN "DBI:mysql:database=zogo;host=127.0.0.1"
       ZOGODbUser "zogo"
       ZOGODbPass "xxx"
       ZOGOGitSmartHttp yes
    </Location>
...
recommended. You should create them with the rights of the user running ZOGO,
like this:

  cd /var/www/git
  sudo -u user-running-zogo mkdir myproject
  cd myproject
  sudo -u user-running-zogo git init --bare
...
my @directives = (
  ...
  {
    name => 'ZOGODbWhereClause',
    req_override => OR_AUTHCFG,
    args_how => TAKE1,
  },
  {
    name => 'ZOGOCacheCredsMax',
    req_override => OR_AUTHCFG,
    args_how => TAKE1,
    errmsg => 'ZOGOCacheCredsMax must be decimal number',
  },
  {
    name => 'ZOGOGitSmartHttp',
    req_override => OR_AUTHCFG,
    args_how => TAKE1,
  },
);
...
sub ZOGODSN {
  ...
  $self->{ZOGOQuery} = trim($query);
}

sub ZOGODbUser { set_val('ZOGODbUser', @_); }
sub ZOGODbPass { set_val('ZOGODbPass', @_); }
sub ZOGODbWhereClause {
  my ($self, $parms, $arg) = @_;
  $self->{ZOGOQuery} = trim($self->{ZOGOQuery}.($arg ? $arg : "")." ");
}

sub ZOGOCacheCredsMax {
  my ($self, $parms, $arg) = @_;
  if ($arg) {
    $self->{ZOGOCachePool} = APR::Pool->new;
    $self->{ZOGOCacheCreds} = APR::Table::make($self->{ZOGOCachePool}, $arg);
    $self->{ZOGOCacheCredsCount} = 0;
    $self->{ZOGOCacheCredsMax} = $arg;
  }
}

sub ZOGOGitSmartHttp {
  my ($self, $parms, $arg) = @_;
  $arg = lc $arg;

  if ($arg eq "yes" || $arg eq "true") {
    $self->{ZOGOGitSmartHttp} = 1;
  } else {
    $self->{ZOGOGitSmartHttp} = 0;
  }
}
...
sub request_is_read_only {
  ...
  if (defined $cfg->{ZOGOGitSmartHttp} and $cfg->{ZOGOGitSmartHttp}) {
...
sub authen_handler {
  ...
  my ($res, $zogo_pass) =  $r->get_basic_auth_pw();
  return $res unless $res == OK;

  if (is_member($r->user, $zogo_pass, $r)) {
      return OK;
  } else {
      $r->note_auth_failure();
      return DECLINED;
...
sub is_member {
  ...
  my $pass_digest = Digest::SHA::sha1_hex($zogo_pass);

  my $access_mode = request_is_read_only($r) ? "R" : "W";

  my $cfg = Apache2::Module::get_config(__PACKAGE__, $r->server, $r->per_dir_config);
  my $usrprojpass;
  if ($cfg->{ZOGOCacheCredsMax}) {
    $usrprojpass = $cfg->{ZOGOCacheCreds}->get($zogo_user.":".$project_id.":".$access_mode);
    return 1 if (defined $usrprojpass and ($usrprojpass eq $pass_digest));
  }
  my $dbh = connect_database($r);
  my $query = $cfg->{ZOGOQuery};
  my $sth = $dbh->prepare($query);
  $sth->execute($zogo_user, $project_id);
  ...
  while (my ($hashed_password, $salt, $auth_source_id, $permissions, $project_status) = $sth->fetchrow_array) {
      ...
      unless ($auth_source_id) {
          ...
          while (my @rowldap = $sthldap->fetchrow_array) {
            ...
            if ($bind_as =~ m/\$login/) {
              # replace $login with $zogo_user and use $zogo_pass
              $bind_as =~ s/\$login/$zogo_user/g;
              $bind_pw = $zogo_pass
            }
            ...
            my $method = $r->method;
            $ret = 1 if ($ldap->authenticate($zogo_user, $zogo_pass) && (($access_mode eq "R" && $permissions =~ /:browse_repository/) || $permissions =~ /:commit_access/));
  ...
  if ($cfg->{ZOGOCacheCredsMax} and $ret) {
    if (defined $usrprojpass) {
      $cfg->{ZOGOCacheCreds}->set($zogo_user.":".$project_id.":".$access_mode, $pass_digest);
    } else {
      if ($cfg->{ZOGOCacheCredsCount} < $cfg->{ZOGOCacheCredsMax}) {
        $cfg->{ZOGOCacheCreds}->set($zogo_user.":".$project_id.":".$access_mode, $pass_digest);
        $cfg->{ZOGOCacheCredsCount}++;
      } else {
        $cfg->{ZOGOCacheCreds}->clear();
        $cfg->{ZOGOCacheCredsCount} = 0;
      }
    }
  }
...
sub get_project_identifier {
    ...
    my $cfg = Apache2::Module::get_config(__PACKAGE__, $r->server, $r->per_dir_config);
    my $location = $r->location;
    $location =~ s/\.git$// if (defined $cfg->{ZOGOGitSmartHttp} and $cfg->{ZOGOGitSmartHttp});
...
sub connect_database {
    ...
    my $cfg = Apache2::Module::get_config(__PACKAGE__, $r->server, $r->per_dir_config);
    return DBI->connect($cfg->{ZOGODSN}, $cfg->{ZOGODbUser}, $cfg->{ZOGODbPass});
}
