...
optparse = OptionParser.new do |opts|
  ...
  opts.separator("")
  opts.separator("Manages your repositories with ZOGO.")
  opts.separator("")
  opts.separator("Required arguments:") 
  opts.on("-s", "--svn-dir DIR",      "use DIR as base directory for svn repositories") {|v| $repos_base = v}
  opts.on("-r", "--zogo-host HOST","assume ZOGO is hosted on HOST. Examples:",
                                       " -r zogo.example.net",
                                       " -r http://zogo.example.net",
                                       " -r https://zogo.example.net") {|v| $zogo_host = v}
  opts.on("-k", "--key KEY",           "use KEY as the ZOGO API key",
  ...
  opts.on("-o", "--owner OWNER",       "owner of the repository. using the rails login", 
                                       ...
                                       "ZOGO even for private projects. If you want to",
                                       "share repositories through ZOGO.pm, you need",
  ...
  opts.on(      "--key-file FILE",     "path to a file that contains the ZOGO API key",
  ...
  opts.on("-f", "--force",             "force repository creation even if the project", "repository is already declared in ZOGO") {$force = true}
  ...
  opts.separator("  reposman.rb --svn-dir=/var/svn --zogo-host=zogo.host")
  opts.separator("  reposman.rb -s /var/git -r zogo.host -u http://git.host --scm git")
  opts.separator("")
  opts.separator("You can find more information on the zogo's wiki:\nhttp://www.zogo.org/projects/zogo/wiki/HowTos")
...
if ($zogo_host.empty? or $repos_base.empty?)
...
class Project < ActiveResource::Base
  self.headers["User-agent"] = "ZOGO repository manager/#{Version}"
  self.format = :json
end

log("querying ZOGO for active projects with repository module enabled...", :level => 1);

$zogo_host.gsub!(/^/, "http://") unless $zogo_host.match("^https?://")
$zogo_host.gsub!(/\/$/, '')

Project.site = "#{$zogo_host}/sys";

begin
  # Get all active projects that have the Repository module enabled
  projects = Project.find(:all, :params => {:key => $api_key})
rescue ActiveResource::ForbiddenAccess
  log("Request was denied by your ZOGO server. Make sure that 'WS for repository management' is enabled in application settings and that you provided the correct API key.")
rescue => e
...
projects.each do |project|
  ...
  if File.directory?(repos_path)
    ...
    # if repository is already declared in zogo, we don't create
    # unless user use -f with reposman
    if $force == false and project.respond_to?(:repository)
      log("\trepository for project #{project.identifier} already exists in ZOGO", :level => 1)
      next
    end
    ...
    if $test
      ...
      log("\trepository #{repos_path} registered in ZOGO with url #{$svn_url}#{project.identifier}") if $svn_url;
      next
    end
    ...
    if $svn_url
      begin
        ...
        log("\trepository #{repos_path} registered in ZOGO with url #{$svn_url}#{project.identifier}");
      rescue => e
        log("\trepository #{repos_path} not registered in ZOGO: #{e.message}");
      end
    end
  end
end
