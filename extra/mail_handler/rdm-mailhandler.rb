...
class ZOGOMailHandler
  ...
  def initialize
    ...
    optparse = OptionParser.new do |opts|
      opts.banner = "Usage: rdm-mailhandler.rb [options] --url=<ZOGO URL> --key=<API key>"
      opts.separator("")
      opts.separator("Reads an email from standard input and forwards it to a ZOGO server through a HTTP request.")
      opts.separator("")
      opts.separator("Required arguments:")
      opts.on("-u", "--url URL",              "URL of the ZOGO server") {|v| self.url = v}
      opts.on("-k", "--key KEY",              "ZOGO API key") {|v| self.key = v}
      opts.separator("")
      opts.separator("General options:")
      opts.on("--key-file FILE",              "full path to a file that contains your ZOGO",
...
Examples:
  ...
  they will be dropped (not recommended):
    ...
    rdm-mailhandler.rb --url http://zogo.domain.foo --key secret

  Fixed project and default tracker specified, but emails can override
  both tracker and priority attributes using keywords:

    rdm-mailhandler.rb --url https://domain.foo/zogo --key secret \\
      --project myproject \\
      --tracker bug \\
      --allow-override tracker,priority

  Project selected by subaddress of zogo@example.net. Sending the email
  to zogo+myproject@example.net will add the issue to myproject:

    rdm-mailhandler.rb --url http://zogo.domain.foo --key secret \\
      --project-from-subaddress zogo@example.net
END_DESC
  ...
  def submit(email)
    ...
    headers = { 'User-Agent' => "ZOGO mail handler/#{VERSION}" }
    ...
    begin
      ...
      warn "An error occurred while contacting your ZOGO server: #{e.message}"
    ...
    case response.code.to_i
      when 403
        warn "Request was denied by your ZOGO server. " +
      ...
      when 422
        ...
        return 77
      when 400..499
        warn "Request was denied by your ZOGO server (#{response.code})."
        return 77
      when 500..599
        warn "Failed to contact your ZOGO server (#{response.code})."
        return 75
      when 201
