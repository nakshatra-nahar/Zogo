  ...
  task :migrate_from_trac => :environment do
    ...
    module TracMigrate
      ...
      def self.convert_wiki_text(text)
        ...
        #      ticket:1234
        #      #1 is working cause ZOGO uses the same syntax.
        text = text.gsub(/ticket\:([^\ ]+)/, '#\1')
        # Milestone links:
        #      [milestone:"0.1.0 Mercury" Milestone 0.1.0 (Mercury)]
        #      The text "Milestone 0.1.0 (Mercury)" is not converted,
        #      cause ZOGO's wiki does not support this.
...
#            TICKET_MAP[$1.to_i] ||= $1
      ...
      def self.migrate
        ...
        TracTicketCustom.find_by_sql("SELECT DISTINCT name FROM #{TracTicketCustom.table_name}").each do |field|
          print '.'
          STDOUT.flush
          # ZOGO custom field name
          field_name = encode(field.name[0, limit_for(IssueCustomField, 'name')]).humanize
          # Find if the custom already exists in ZOGO
        ...
        # Trac 'resolution' field as a ZOGO custom field
      ...
      def self.target_project_identifier(identifier)
        ...
        if !project
          ...
          puts
          puts "This project already exists in your ZOGO database."
    ...
    puts
    if ZOGO::DefaultData::Loader.no_data?
      puts "ZOGO configuration need to be loaded before importing data."
      puts "Please, run this first:"
      puts
      puts "  rake zogo:load_default_data RAILS_ENV=\"#{ENV['RAILS_ENV']}\""
      exit
    end

    puts "WARNING: a new project will be added to ZOGO during this process."
