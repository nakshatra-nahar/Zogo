...
namespace :zogo do
  namespace :attachments do
    ...
    desc 'Moves attachments stored at the root of the file directory (ie. created before ZOGO 2.3) to their subdirectories'
  ...
  task :plugins do
    Rake::Task["zogo:plugins:migrate"].invoke
    Rake::Task["zogo:plugins:assets"].invoke
  end

desc <<-DESC
FOR EXPERIMENTAL USE ONLY, Moves ZOGO data from production database to the development database.
...
DESC
  ...
  namespace :plugins do
    ...
    task :migrate => :environment do
      ...
      begin
        ZOGO::Plugin.migrate(name, version)
      rescue ZOGO::PluginNotFound
    ...
    task :assets => :environment do
      ...
      begin
        ZOGO::PluginLoader.mirror_assets(name)
      rescue ZOGO::PluginNotFound
        abort "Plugin #{name} was not found."
      end
    end
    ...
    task :test do
      Rake::Task["zogo:plugins:test:units"].invoke
      Rake::Task["zogo:plugins:test:functionals"].invoke
      Rake::Task["zogo:plugins:test:integration"].invoke
      Rake::Task["zogo:plugins:test:system"].invoke
    end
