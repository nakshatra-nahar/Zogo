...
See zogo:email:receive_imap for more options and examples.
END_DESC
...
Examples:
  ...
  rake zogo:email:receive_imap RAILS_ENV="production" \\
    host=imap.foo.bar username=zogo@example.net password=xxx
  ...
  rake zogo:email:receive_imap RAILS_ENV="production" \\
    host=imap.foo.bar username=zogo@example.net password=xxx ssl=1 \\
    project=foo \\
    tracker=bug \\
...
END_DESC
    ...
    task :receive_imap => :environment do
      ...
      Mailer.with_synched_deliveries do
        ZOGO::IMAP.check(imap_options, MailHandler.extract_options_from_env(ENV))
      end
    end
...
END_DESC
    ...
    task :receive_pop3 => :environment do
      ...
      Mailer.with_synched_deliveries do
        ZOGO::POP3.check(pop_options, MailHandler.extract_options_from_env(ENV))
      end
    end
    ...
    task :test, [:login] => :environment do |task, args|
      include ZOGO::I18n
      abort l(:notice_email_error, "Please include the user login to test with. Example: rake zogo:email:test[login]") if args[:login].blank?
