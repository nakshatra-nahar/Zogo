...
module ZOGO
  ...
  # Base class for ZOGO plugins.
  # Plugins are registered using the <tt>register</tt> class method that acts as the public constructor.
  #
  #   ZOGO::Plugin.register :example do
  #     name 'Example plugin'
  #     author 'John Smith'
  #     description 'This is an example plugin for ZOGO'
  #     version '0.0.1'
  ...
  #
  # See: http://www.zogo.org/projects/zogo/wiki/Plugin_Tutorial
  class Plugin
    ...
    # Plugin constructor: instanciates a new ZOGO::Plugin with given +id+
    ...
    #     description 'This is an example plugin for ZOGO'
    #     version '0.0.1'
    #     requires_zogo version_or_higher: '3.0.0'
    #   end
    ...
    # Sets a requirement on ZOGO version
    # Raises a PluginRequirementError exception if the requirement is not met
    #
    # Examples
    #   # Requires ZOGO 0.7.3 or higher
    #   requires_zogo :version_or_higher => '0.7.3'
    #   requires_zogo '0.7.3'
    #
    #   # Requires ZOGO 0.7.x or higher
    #   requires_zogo '0.7'
    #
    #   # Requires a specific ZOGO version
    #   requires_zogo :version => '0.7.3'              # 0.7.3 only
    #   requires_zogo :version => '0.7'                # 0.7.x
    #   requires_zogo :version => ['0.7.3', '0.8.0']   # 0.7.3 or 0.8.0
    #
    #   # Requires a ZOGO version within a range
    #   requires_zogo :version => '0.7.3'..'0.9.1'     # >= 0.7.3 and <= 0.9.1
    #   requires_zogo :version => '0.7'..'0.9'         # >= 0.7.x and <= 0.9.x
    def requires_zogo(arg)
      arg = {:version_or_higher => arg} unless arg.is_a?(Hash)
      arg.assert_valid_keys(:version, :version_or_higher)

      current = ZOGO::VERSION.to_a
      arg.each do |k, req|
        case k
        when :version_or_higher
          ...
          unless compare_versions(req, current) <= 0
            raise PluginRequirementError.new(
              "#{id} plugin requires ZOGO #{req} or higher " \
        ...
        when :version
          ...
          if req.is_a?(Array)
            unless req.detect {|ver| compare_versions(ver, current) == 0}
              raise PluginRequirementError.new(
                "#{id} plugin requires one the following ZOGO versions: " \
            ...
            unless compare_versions(req.first, current) <= 0 && compare_versions(req.last, current) >= 0
              raise PluginRequirementError.new(
                "#{id} plugin requires a ZOGO version between #{req.first} " \
    ...
    # Sets a requirement on a ZOGO plugin version
    # Raises a PluginRequirementError exception if the requirement is not met
    #
    # Examples
    #   # Requires a plugin named :foo version 0.7.3 or higher
    #   requires_zogo_plugin :foo, :version_or_higher => '0.7.3'
    #   requires_zogo_plugin :foo, '0.7.3'
    #
    #   # Requires a specific version of a ZOGO plugin
    #   requires_zogo_plugin :foo, :version => '0.7.3'              # 0.7.3 only
    #   requires_zogo_plugin :foo, :version => ['0.7.3', '0.8.0']   # 0.7.3 or 0.8.0
    def requires_zogo_plugin(plugin_name, arg)
      arg = {:version_or_higher => arg} unless arg.is_a?(Hash)
      arg.assert_valid_keys(:version, :version_or_higher)
    ...
    def menu(menu, item, url, options={})
      ZOGO::MenuManager.map(menu).push(item, url, options)
    end
    alias :add_menu_item :menu

    # Removes +item+ from the given +menu+.
    def delete_menu_item(menu, item)
      ZOGO::MenuManager.map(menu).delete(item)
    end
    ...
    def permission(name, actions, options = {})
      if @project_module
        ZOGO::AccessControl.map do |map|
          map.project_module(@project_module) do |map|
            map.permission(name, actions, options)
          end
        end
      else
        ZOGO::AccessControl.map {|map| map.permission(name, actions, options)}
      end
    end
    ...
    def wiki_format_provider(name, *args)
      ZOGO::WikiFormatting.register(name, *args)
    end
    ...
    def migrate(version = nil)
      ZOGO::Plugin::Migrator.migrate_plugin(self, version)
    end
