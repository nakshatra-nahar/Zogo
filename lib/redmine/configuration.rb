...
module ZOGO
  module Configuration
    ...
    class << self
      # Loads the ZOGO configuration file
      # Valid options:
      ...
      def load(options={})
        ...
        if @config['email_delivery']
          ...
          @config['email_delivery'].each do |k, v|
            ...
            # delivery methods that are removed in ZOGO 4.0.
            if k == 'delivery_method' && v.to_s =~ /\Aasync_(.+)/
              abort "ZOGO now uses ActiveJob to send emails asynchronously and the :#{v} delivery method is no longer available.\n" +
      ...
      def load_from_yaml(filename, env)
        ...
        begin
          ...
          abort "Your ZOGO configuration file located at #{filename} is not a valid YAML file and could not be loaded:\n#{e.message}"
        rescue SyntaxError => e
          abort "A syntax error occurred when parsing your ZOGO configuration file located at #{filename} with ERB:\n#{e.message}"
        end
        conf = {}
        if yaml.is_a?(Hash)
          if yaml['default']
          ...
          abort "Your ZOGO configuration file located at #{filename} is not a valid ZOGO configuration file."
        end
        conf
      end
        ...
        @config.each do |name, value|
          if value.present? && /^scm_.+_path_regexp$/.match?(name)
            begin
              ...
              abort "Invalid regular expression set as #{name} setting in your ZOGO configuration file:\n#{e.message}"
            end
          end
        end
      end
