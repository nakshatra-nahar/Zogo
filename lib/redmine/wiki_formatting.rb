  ...
  module WikiFormatting
    ...
    class << self
      ...
      def register(name, *args)
        ...
        formatter, helper, parser =
          if args.any?
            ...
            %w(Formatter Helper HtmlParser).map {|m| "ZOGO::WikiFormatting::#{name.classify}::#{m}".constantize rescue nil}
          end
      ...
      def formatter_for(name)
        ...
        (entry && entry[:formatter]) || ZOGO::WikiFormatting::NullFormatter::Formatter
      end

      def helper_for(name)
        entry = @@formatters[name.to_s]
        (entry && entry[:helper]) || ZOGO::WikiFormatting::NullFormatter::Helper
      end

      def html_parser_for(name)
        entry = @@formatters[name.to_s]
        (entry && entry[:html_parser]) || ZOGO::WikiFormatting::HtmlParser
      end
