...
module ZOGO
  module Helpers
    ...
    class Gantt
      ...
      include ERB::Util
      include ZOGO::I18n
      include ZOGO::Utils::DateCalculation
      ...
      def to_image(format='PNG')
        ...
        unless ZOGO::Configuration['rmagick_font_path'].nil?
          Rails.logger.warn(
            'rmagick_font_path option is deprecated. Use minimagick_font_path instead.'
          )
        end
        font_path =
          ZOGO::Configuration['minimagick_font_path'].presence ||
            ZOGO::Configuration['rmagick_font_path'].presence
        img = MiniMagick::Image.create(".#{format}", false)
        if ZOGO::Configuration['imagemagick_convert_command'].present?
          MiniMagick.cli_path = File.dirname(ZOGO::Configuration['imagemagick_convert_command'])
        end
        MiniMagick::Tool::Convert.new do |gc|
          ...
          @months.times do
            ...
            gc.strokewidth(1)
            gc.draw('text %d,%d %s' % [
              left.round + 8, 14, ZOGO::Utils::Shell.shell_quote("#{month_f.year}-#{month_f.month}")
            ])
            left = left + width
            month_f = month_f >> 1
          end
          ...
          if show_weeks
            ...
            while week_f <= date_to
              ...
              gc.strokewidth(1)
              gc.draw('text %d,%d %s' % [
                left.round + 2, header_height + 14, ZOGO::Utils::Shell.shell_quote(week_f.cweek.to_s)
              ])
              left = left + width
              week_f = week_f + 7
            end
      ...
      def to_pdf
        pdf = ::ZOGO::Export::PDF::ITCPDF.new(current_language)
      ...
      def image_subject(params, subject, options={})
        ...
        params[:image].draw('text %d,%d %s' % [
          params[:indent], params[:top] + 2, ZOGO::Utils::Shell.shell_quote(subject)
        ])
      end
      ...
      def image_task(params, coords, markers, label, object)
        ...
        if label
          ...
          params[:image].draw('text %d,%d %s' % [
            ...
            params[:top] + 1,
            ZOGO::Utils::Shell.shell_quote(label)
          ])
        end
      end
    end
