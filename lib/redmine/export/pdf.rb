    ...
    module PDF
      class ITCPDF < RBPDF
        ...
        def initialize(lang, orientation='P')
          ...
          set_creator(ZOGO::Info.app_name)
        ...
        def formatted_text(text)
          ZOGO::WikiFormatting.to_html(Setting.text_formatting, text)
        end
      ...
      class RDMPdfEncoding
        def self.rdm_from_utf8(txt, encoding)
          txt ||= ''
          ZOGO::CodesetUtil.from_utf8(txt, encoding).b
        end

        def self.attach(attachments, filename, encoding)
          filename_utf8 = ZOGO::CodesetUtil.to_utf8(filename, encoding)
          atta = nil
