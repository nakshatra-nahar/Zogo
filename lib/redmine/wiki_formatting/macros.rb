    ...
    module Macros
      module Definitions
        ...
        def macro_exists?(name)
          ZOGO::WikiFormatting::Macros.available_macros.key?(name.to_sym)
        end

        def exec_macro(name, obj, args, text, options={})
          macro_options = ZOGO::WikiFormatting::Macros.available_macros[name.to_sym]
          return unless macro_options

          if options[:inline_attachments] == false
            ZOGO::WikiFormatting::Macros.inline_attachments = false
          else
            ZOGO::WikiFormatting::Macros.inline_attachments = true
          end
      ...
      macro :collapse do |obj, args, text|
        html_id = "collapse-#{ZOGO::Utils.random_hex(4)}"
