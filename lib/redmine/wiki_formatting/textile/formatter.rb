  ...
  module WikiFormatting
    module Textile
      class Formatter < RedCloth3
        ...
        RULES = [:textile, :block_markdown_rule, :inline_auto_link, :inline_auto_mailto, :inline_restore_zogo_links]
          ...
          unless @pre_list.empty?
            ...
            text.gsub!(/<redpre#(\d+)>/) do
              ...
              if content.match(/<code\s+class=(?:"([^"]+)"|'([^']+)')>\s?(.*)/m)
                ...
                if ZOGO::SyntaxHighlighting.language_supported?(language)
                  text.gsub!(/x%x%/, '&')
                  content = "<code class=\"#{CGI.escapeHTML language} syntaxhl\"#{langattr}>" +
                    ZOGO::SyntaxHighlighting.highlight_by_language(text, language)
                else
