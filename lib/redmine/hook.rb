...
module ZOGO
  module Hook
    ...
    class << self
      ...
      # Automatically called when a class inherits from ZOGO::Hook::Listener.
    ...
    module Helper
      def call_hook(hook, context={})
        if is_a?(ActionController::Base)
          ...
          ZOGO::Hook.call_hook(hook, default_context.merge(context))
        else
          ...
          ZOGO::Hook.call_hook(hook, default_context.merge(context)).join(' ').html_safe
        end
      end
    end
  end
