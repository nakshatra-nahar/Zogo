...
class Mailer < ActionMailer::Base
  ...
  def issue_add(user, issue)
    zogo_headers 'Project' => issue.project.identifier,
  ...
  def document_added(user, document, author)
    zogo_headers 'Project' => document.project.identifier
    @author = author
    @document = document
    @user = user
  ...
  def attachments_added(user, attachments)
    ...
    zogo_headers 'Project' => container.project.identifier
  ...
  def news_added(user, news)
    zogo_headers 'Project' => news.project.identifier
    @author = news.author
    message_id news
    references news
    @news = news
  ...
  def message_posted(user, message)
    zogo_headers 'Project' => message.project.identifier,
  ...
  def wiki_content_added(user, wiki_content)
    zogo_headers 'Project' => wiki_content.project.identifier,
  ...
  def security_notification(user, sender, options={})
    @sender = sender
    zogo_headers 'Sender' => sender.login
    @message =
      l(options[:message],
  ...
  def mail(headers={}, &block)
    ...
    headers.reverse_merge! 'X-Mailer' => 'ZOGO',
            'X-ZOGO-Host' => Setting.host_name,
            'X-ZOGO-Site' => Setting.app_title,
    ...
    if @author&.logged?
      zogo_headers 'Sender' => @author.login
    end
  ...
  # Appends a ZOGO header field (name is prepended with 'X-ZOGO-')
  def zogo_headers(h)
    h.compact.each {|k, v| headers["X-ZOGO-#{k}"] = v.to_s}
  end
  ...
  class << self
    def token_for(object, user)
      ...
      host = "#{::Socket.gethostname}.zogo" if host.empty?
      "#{hash.join('.')}@#{host}"
    end
