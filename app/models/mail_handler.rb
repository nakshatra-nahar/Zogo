...
class MailHandler < ActionMailer::Base
  ...
  def receive(email, options={})
    ...
    if sender_email.casecmp(emission_address) == 0
      logger&.info "MailHandler: ignoring email from ZOGO emission address [#{sender_email}]"
      return false
    end
  ...
  MESSAGE_ID_RE = %r{^<?zogo\.([a-z0-9_]+)\-(\d+)\.\d+(\.[a-f0-9]+)?@}
  ...
  def receive_issue
    ...
    # add To and Cc as watchers before saving so the watchers can reply to ZOGO
    add_watchers(issue)
    issue.save!
  ...
  def email_parts_to_text(parts)
    ...
    parts.map do |p|
      ...
      body = ZOGO::CodesetUtil.to_utf8(p.body.decoded, body_charset)
  ...
  class << self
    ...
    def html_body_to_text(html)
      ZOGO::WikiFormatting.html_parser.to_text(html)
    end
    ...
    def new_user_from_attributes(email_address, fullname=nil)
      ...
      unless user.valid?
        user.login = "user#{ZOGO::Utils.random_hex(6)}" unless user.errors[:login].blank?
  ...
  def cleanup_body(body)
    ...
    unless delimiters.empty?
      ...
      if Setting.text_formatting == "common_mark" && ZOGO::Configuration['common_mark_enable_hardbreaks'] == false
        body = ZOGO::WikiFormatting::CommonMark::AppendSpacesToLines.call(body)
      end
