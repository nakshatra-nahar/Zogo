...
class Attachment < ActiveRecord::Base
  ...
  @@storage_path = ZOGO::Configuration['attachments_storage_path'] || File.join(Rails.root, "files")
  ...
  def files_to_final_location
    ...
    if content_type.blank? && filename.present?
      self.content_type = ZOGO::MimeType.of(filename)
    end
  ...
  def thumbnailable?
    ZOGO::Thumbnail.convert_available? && (
      image? || (is_pdf? && ZOGO::Thumbnail.gs_available?)
    )
  end
  ...
  def thumbnail(options={})
    if thumbnailable? && readable?
      ...
      begin
        ZOGO::Thumbnail.generate(self.diskfile, target, size, is_pdf?)
      rescue => e
        if logger
          logger.error(
  ...
  def is_text?
    ZOGO::MimeType.is_type?('text', filename) || ZOGO::SyntaxHighlighting.filename_supported?(filename)
  end

  def is_markdown?
    ZOGO::MimeType.of(filename) == 'text/markdown'
  end

  def is_textile?
    ZOGO::MimeType.of(filename) == 'text/x-textile'
  end

  def is_image?
    ZOGO::MimeType.is_type?('image', filename)
  end
  ...
  def is_pdf?
    ZOGO::MimeType.of(filename) == "application/pdf"
  end

  def is_video?
    ZOGO::MimeType.is_type?('video', filename)
  end

  def is_audio?
    ZOGO::MimeType.is_type?('audio', filename)
  end
  ...
  # directory (ie. created before ZOGO 2.3) to their target subdirectories
