...
class TimestampQueryColumn < QueryColumn
  ...
  def group_by_statement
    ZOGO::Database.timestamp_to_date(sortable, User.current.time_zone)
  end
...
class Query < ActiveRecord::Base
  ...
  def group_by_sort_order
    if column = group_by_column
      ...
      if column.is_a?(TimestampQueryColumn)
        column_sortable = ZOGO::Database.timestamp_to_date(column.sortable, User.current.time_zone)
      end
  ...
  def sql_contains(db_field, value, options={})
    ...
    if prefix || suffix
      ...
      queried_class.sanitize_sql_for_conditions(
        [ZOGO::Database.like(db_field, '?', :match => options[:match]), "#{prefix}#{value}#{suffix}"]
      )
    else
      queried_class.sanitize_sql_for_conditions(
  ...
  def self.tokenized_like_conditions(db_field, value, **options)
    tokens = ZOGO::Search::Tokenizer.new(value).tokens
    tokens = [value] unless tokens.present?
    sql, values = tokens.map do |token|
      [ZOGO::Database.like(db_field, '?', options), "%#{sanitize_sql_like token}%"]
    end.transpose
    [sql.join(" AND "), *values]
  end
