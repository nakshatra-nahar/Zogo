...
class Role < ActiveRecord::Base
  ...
  def setable_permissions
    setable_permissions = ZOGO::AccessControl.permissions - ZOGO::AccessControl.public_permissions
    setable_permissions -= ZOGO::AccessControl.members_only_permissions if self.builtin == BUILTIN_NON_MEMBER
    setable_permissions -= ZOGO::AccessControl.loggedin_only_permissions if self.builtin == BUILTIN_ANONYMOUS
    setable_permissions
  end
  ...
  def allowed_permissions
    @allowed_permissions ||= permissions + ZOGO::AccessControl.public_permissions.collect {|p| p.name}
  end

  def allowed_actions
    @actions_allowed ||=
      allowed_permissions.inject([]) do |actions, permission|
        actions += ZOGO::AccessControl.allowed_actions(permission)
      end.flatten
  end
