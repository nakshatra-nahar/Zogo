...
class UserPreference < ActiveRecord::Base
  ...
  def toolbar_language_options=(value)
    languages =
      value.to_s.delete(' ').split(',').select do |lang|
        ZOGO::SyntaxHighlighting.language_supported?(lang)
      end.compact
  ...
  def my_page_layout
    self[:my_page_layout] ||= ZOGO::MyPage.default_layout.deep_dup
  end
  ...
  def add_block(block)
    block = block.to_s.underscore
    return unless ZOGO::MyPage.valid_block?(block, my_page_layout.values.flatten)
  ...
  def order_blocks(group, blocks)
    group = group.to_s
    if ZOGO::MyPage.groups.include?(group) && blocks.present?
