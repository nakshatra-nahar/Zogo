...
class Repository < ActiveRecord::Base
  ...
  # Returns the ZOGO User corresponding to the given +committer+
  ...
  def self.fetch_changesets
    Project.active.has_module(:repository).all.each do |project|
      project.repositories.each do |repository|
        ...
        rescue ZOGO::Scm::Adapters::CommandFailed => e
  ...
  def self.repository_class(class_name)
    ...
    if ZOGO::Scm::Base.all.include?(class_name)
  ...
  # that can be set in the ZOGO configuration file.
  def validate_repository_path(attribute=:url)
    regexp = ZOGO::Configuration["scm_#{scm_name.to_s.downcase}_path_regexp"]
