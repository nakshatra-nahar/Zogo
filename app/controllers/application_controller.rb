...
class ApplicationController < ActionController::Base
  ...
  include ZOGO::Search::Controller
  include ZOGO::MenuManager::MenuController
  helper ZOGO::MenuManager::MenuHelper

  include ZOGO::SudoMode::Controller

  def session_expiration
    if session[:user_id] && Rails.application.config.zogo_verify_sessions != false
    ...
    if user.nil? && Setting.rest_api_enabled? && accept_api_auth?
      ...
      if user && user.admin? && (username = api_switch_user_from_request)
        ...
        if su && su.active?
          ...
          render_error :message => 'Invalid X-ZOGO-Switch-User header', :status => 412
        end
      end
    end
  ...
  def autologin_cookie_name
    ZOGO::Configuration['autologin_cookie_name'].presence || 'autologin'
  end
  ...
  def check_twofa_activation
    if session[:must_activate_twofa]
      if User.current.must_activate_twofa?
        ...
        if ZOGO::Twofa.available_schemes.length == 1
          twofa_scheme = ZOGO::Twofa.for_twofa_scheme(ZOGO::Twofa.available_schemes.first)
      ...
      respond_to do |format|
        ...
        format.api do
          if Setting.rest_api_enabled? && accept_api_auth?
            head(:unauthorized, 'WWW-Authenticate' => 'Basic realm="ZOGO API"')
          else
            head(:forbidden)
          end
        end
        format.js   {head :unauthorized, 'WWW-Authenticate' => 'Basic realm="ZOGO API"'}
  ...
  def record_project_usage
    if @project && @project.id && User.current.logged? && User.current.allowed_to?(:view_project, @project)
      ZOGO::ProjectJumpBox.new(User.current).project_used(@project)
    end
    true
  end
  ...
  def api_key_from_request
    if params[:key].present?
      params[:key].to_s
    elsif request.headers["X-ZOGO-API-Key"].present?
      request.headers["X-ZOGO-API-Key"].to_s
    end
  end

  # Returns the API 'switch user' value if present
  def api_switch_user_from_request
    request.headers["X-ZOGO-Switch-User"].to_s.presence
  end
