...
class AdminController < ApplicationController
  ...
  def index
    @no_configuration_data = ZOGO::DefaultData::Loader::no_data?
  end
  ...
  def default_configuration
    if request.post?
      begin
        ZOGO::DefaultData::Loader::load(params[:lang])
  ...
  def test_email
    begin
      ...
      flash[:error] = l(:notice_email_error, ERB::Util.h(ZOGO::CodesetUtil.replace_invalid_utf8(e.message.dup)))
    end
  ...
  def info
    @checklist = [
      ...
      ["#{l :text_plugin_assets_writable} (./public/plugin_assets)",   File.writable?(ZOGO::Plugin.public_directory)],
      ...
      [:text_convert_available,        ZOGO::Thumbnail.convert_available?],
      [:text_gs_available,             ZOGO::Thumbnail.gs_available?]
    ]
  end
end
