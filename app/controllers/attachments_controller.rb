...
class AttachmentsController < ApplicationController
      ...
      format.html do
        if @attachment.container.respond_to?(:attachments)
          ...
          if index = @attachments.index(@attachment)
            @paginator = ZOGO::Pagination::Paginator.new(
  ...
  def upload
    ...
    @attachment.filename = params[:filename].presence || ZOGO::Utils.random_hex(16)
  ...
  def download_all
    ...
    if zip_data
      ...
      send_data(
        zip_data,
        :type => ZOGO::MimeType.of(file_name),
        :filename => file_name
      )
    else
      render_404
  ...
  def detect_content_type(attachment, is_thumb = false)
    ...
    if content_type.blank? || content_type == "application/octet-stream"
      content_type =
        ZOGO::MimeType.of(attachment.filename).presence ||
