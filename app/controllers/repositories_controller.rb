...
class RepositoriesController < ApplicationController
  ...
  rescue_from ZOGO::Scm::Adapters::CommandFailed, :with => :show_error_command_failed
  ...
  def entry_and_raw(is_raw)
    ...
    if is_raw
      ...
      send_type = ZOGO::MimeType.of(@path)
      ...
      if index = @entries.index{|e| e.name == @entry.name}
        @paginator = ZOGO::Pagination::Paginator.new(@entries.size, 1, index+1)
      end
  ...
  def is_entry_text_data?(ent, path)
    ...
    return true if ZOGO::MimeType.is_type?('text', path)
    # Ruby 1.8.6 has a bug of integer divisions.
    # http://apidock.com/ruby/v1_8_6_287/String/is_binary_data%3F
    return false if ZOGO::Scm::Adapters::ScmData.binary?(ent)
  ...
  def build_new_repository_from_params
    scm = params[:repository_scm] || (ZOGO::Scm::Base.all & Setting.enabled_scm).first
  ...
  # Handler for ZOGO::Scm::Adapters::CommandFailed exception
  ...
  def disposition(path)
    if ZOGO::MimeType.of(@path) == "application/pdf"
      'inline'
    else
      'attachment'
    end
