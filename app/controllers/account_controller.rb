...
class AccountController < ApplicationController
  ...
  def twofa_setup
    ...
    @twofa = ZOGO::Twofa.for_user(@user)
  end
  ...
  def set_autologin_cookie(user)
    ...
    secure = ZOGO::Configuration['autologin_cookie_secure']
    if secure.nil?
      secure = request.ssl?
    end
    cookie_options = {
      ...
      :path => (ZOGO::Configuration['autologin_cookie_path'] || ZOGOApp::Application.config.relative_url_root || '/'),
      :same_site => :lax,
      :secure => secure,
      :httponly => true
    }
