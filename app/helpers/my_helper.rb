...
module MyHelper
  ...
  def render_block_content(block, user)
    unless block_definition = ZOGO::MyPage.find_block(block)
  ...
  def block_select_tag(user)
    ...
    ZOGO::MyPage.block_options(blocks_in_use).each do |label, block|
  ...
  def render_calendar_block(block, settings)
    calendar = ZOGO::Helpers::Calendar.new(User.current.today, current_language, :week)
  ...
  def render_activity_block(block, settings)
    events_by_day = ZOGO::Activity::Fetcher.new(User.current, :author => User.current).events(nil, nil, :limit => 10).group_by {|event| User.current.time_to_date(event.event_datetime)}
