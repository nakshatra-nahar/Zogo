...
module ApplicationHelper
  include ZOGO::WikiFormatting::Macros::Definitions
  include ZOGO::I18n
  include ZOGO::Pagination::Helper
  include ZOGO::SudoMode::Helper
  include ZOGO::Themes::Helper
  include ZOGO::Hook::Helper
  include ZOGO::Helpers::URL
  ...
  def render_projects_for_jump_box(projects, selected: nil, query: nil)
    if query.blank?
      jump_box = ZOGO::ProjectJumpBox.new User.current
  ...
  def syntax_highlight(name, content)
    ZOGO::SyntaxHighlighting.highlight_by_filename(content, name)
  end
  ...
  def other_formats_links(&block)
    ...
    yield ZOGO::Views::OtherFormatsBuilder.new(self)
    concat('</p>'.html_safe)
  end
  ...
  # Exemples:
  #   html_title 'Foo', 'Bar'
  #   html_title # => 'Foo - Bar - My Project - ZOGO'
  def html_title(*args)
    if args.empty?
      title = @html_title || []
  ...
  def body_css_classes
    css = []
    if theme = ZOGO::Themes.theme(Setting.ui_theme)
  ...
  def textilizable(*args)
    ...
    if options[:formatting] == false
      ...
      text = ZOGO::WikiFormatting.to_html(formatting, text, :object => obj, :attribute => attr)
    end
    ...
    text = parse_non_pre_blocks(text, obj, macros, options) do |text|
      [:parse_inline_attachments, :parse_hires_images, :parse_wiki_links, :parse_zogo_links].each do |method_name|
  ...
  def parse_zogo_links(text, default_project, obj, attr, only_path, options)
  ...
  def labelled_form_for(*args, &proc)
    ...
    options[:builder] = ZOGO::Views::LabelledFormBuilder
    form_for(*args, &proc)
  end
  ...
  def include_calendar_headers_tags
    unless @calendar_headers_tags_included
      ...
      content_for :header_tags do
        ...
        # ZOGO uses 1..7 (monday..sunday) in settings and locales
  ...
  # Returns options or nil if nometa param or X-ZOGO-Nometa header
  # was set in the request
  def api_meta(options)
    if params[:nometa].present? || request.headers['X-ZOGO-Nometa']
  ...
  def wiki_helper
    helper = ZOGO::WikiFormatting.helper_for(Setting.text_formatting)
    extend helper
    return self
  end
